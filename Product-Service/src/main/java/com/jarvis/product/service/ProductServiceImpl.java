...
public class ProductServiceImpl implements ProductService {
	...
	public ProductResponseDTO createProduct(ProductRequestDTO dto) {
		...
		repository.findByNameAndDeletedFalse(dto.getName()).ifPresent(p -> {
	...
	public ProductResponseDTO getProductById(Long id) {
		Products product = repository.findById(id).filter(p -> !p.isDeleted())
	...
	private Products getEntity(Long id) {
		return repository.findById(id).filter(p -> !p.isDeleted())
				.orElseThrow(() -> new ResourceNotFoundException("Product not found"));
	}
	...
	@Override
	public Page<ProductResponseDTO> getAllProducts(int page, int size, String sortBy, String sortDir, String category,
		...
		if (category != null) {
			return repository.findByCategoryAndDeletedFalse(category, pageable).map(this::mapToDTO);
		}

		if (minPrice != null && maxPrice != null) {
			return repository.findByPriceBetweenAndDeletedFalse(minPrice, maxPrice, pageable).map(this::mapToDTO);
		}

		if (minQty != null && maxQty != null) {
			return repository.findByQuantityBetweenAndDeletedFalse(minQty, maxQty, pageable).map(this::mapToDTO);
		}

		return repository.findAllByDeletedFalse(pageable).map(this::mapToDTO);
	}
